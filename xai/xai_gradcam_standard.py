...
class GradCAMStandard:
    ...
    def generate(self, input_tensor: torch.Tensor, class_idx: Optional[int] = None) -> np.ndarray:
        ...
        # Standard Grad-CAM: global-average-pool gradients over HxW
...
def save_cam_overlay(image_path: str, cam: np.ndarray, out_prefix: str) -> None:
    ...
    overlay.save(os.path.join(OUTPUT_DIR, f"{out_prefix}_overlay_std.png"))
    cam_resized.save(os.path.join(OUTPUT_DIR, f"{out_prefix}_heatmap_std.png"))
...
def main() -> None:
    ...
    cam = GradCAMStandard(model, target_layer)
    ...
    for image_path in image_paths:
        ...
        print(f"Saved Standard Grad-CAM to: {OUTPUT_DIR}")
